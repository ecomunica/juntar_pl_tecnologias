...
janela = tb.Window(themename="flatly")  # Alterar tema para 'flatly' para um visual mais moderno
...
# Label para mostrar o diretório selecionado, com fonte personalizada
label_diretorio = ttk.Label(janela, text="Nenhum diretório selecionado", font=("Helvetica", 12, "italic"))
label_diretorio.grid(row=0, column=0, padx=10, pady=10, sticky="ew")

# Botão para selecionar o diretório com padding, cor e tamanho ajustado
botao_diretorio = ttk.Button(janela, text="Selecionar Diretório", command=selecionar_diretorio, bootstyle="info")
...
botao_iniciar = ttk.Button(janela, text="Iniciar Processamento", command=iniciar_processamento, state=tk.DISABLED, bootstyle="success")
botao_iniciar.grid(row=2, column=0, pady=10, padx=80, ipadx=10, sticky="ew")

# Barra de progresso estilizada com tema "info-striped"
progress_bar = ttk.Progressbar(janela, orient="horizontal", mode="indeterminate", bootstyle="info-striped")
progress_bar.grid(row=3, column=0, pady=10, padx=80, sticky="ew")

# Campo de texto para mostrar os resultados
texto_resultado = tk.Text(janela, height=10, width=60, font=("Arial", 10))
